...
class Schwarzschild:
    ...
    #         for j in range(4):
    #             for k in range(4):
    #                 term1 = 0.5 * (sym.diff(self.g[i, j], coord_list[k]) +
    #                                sym.diff(self.g[i, k], coord_list[j]) -
    #                                sym.diff(self.g[j, k], coord_list[i]))
    #                 term2 = 0
    #                 for m in range(4):
    #                     term2 += 0.5 * self.g[i, m] * (sym.diff(self.g[m, j], coord_list[k]) +
    #                                                    sym.diff(self.g[m, k], coord_list[j]) -
    #                                                    sym.diff(self.g[j, k], coord_list[m]))
    #                 christoffel_symbols.append(term1 + term2)
    #
    #     christoffel_symbols = np.array(christoffel_symbols)
    #     christoffel_symbols = np.reshape(christoffel_symbols, (4, 4, 4))
    #
        ...
        for i in range(num_steps):
            for j in range(4):
                trajectory[i + 1, j] = trajectory[i, j] + step_size * geodesic_equations[j].subs(
                    [(self.t, trajectory[i, 0]), (self.r, trajectory[i, 1]), (self.theta, trajectory[i, 2]),
                     (self.phi, trajectory[i, 3])])

        # print(trajectory)
        return trajectory


schwarzschild = Schwarzschild(1, 6.67408e-11, 1.989e12)
# schwarzschild.print_metric()
schwarzschild.calc_christoffel()
# schwarzschild.calc_geodesic_equations()
# schwarzschild.calc_geodesic([120, 12, 4, 7], 0.1, 1000)
