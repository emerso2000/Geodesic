...
void main() {
    if (gl_GlobalInvocationID.x >= 1000000) return;
    ...
    vr += accel_r * position.w * dt;
    vt += accel_theta * position.w * dt;
    position.w += velocity.w * position.w / r * dt; 
    r += vr * position.w * dt;
    theta += vt * position.w / r * dt;
    phi += velocity.w * position.w / (r * r * sin(theta)) * dt;
